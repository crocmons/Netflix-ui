...
export const fetchDataGenre = createAsyncThunk("netflix/moviesGenre", async ({genre,type},thunkApi)=>{
    const {netflix:{genres}} = thunkApi.getState();
...
export const getUserLikedMovies = createAsyncThunk("netflix/getLiked",async (email)=>{
   const {data:{movies}} = await axios.get(`https://netflixapp40.herokuapp.com/api/user/liked/${email}`);
   return movies;
})

export const removeLikedMovies = createAsyncThunk("netflix/deleteLiked",async ({email,movieId})=>{
     const {data:{movies}} = await axios.put("https://netflixapp40.herokuapp.com/api/user/delete",{
        email,
        movieId
     });
     return movies;
...
const NetflixSlice = createSlice({
    ...
    extraReducers:(builder)=>{
        ...
        builder.addCase(fetchDataGenre.fulfilled,(state,action)=>{
            state.movies = action.payload;
        });
